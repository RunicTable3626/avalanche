...
- [ ] **T01** — Frontend config: `package.json` (tauri 2.x, @tauri-apps/api, @tauri-apps/plugin-store, @tauri-apps/plugin-notification, @solidjs/router, vite, vite-plugin-solid, solid-js, typescript, jsQR — pin exact versions), `tsconfig.json` (`strict: true`, `jsx: "preserve"`, `jsxImportSource: "solid-js"`), `vite.config.ts` (solidPlugin + Tauri env).
