...
pub async fn create(conn: &mut PgConnection, did: &str) -> Result<i64, sqlx::Error> {
    let row = sqlx::query("INSERT INTO accounts (did) VALUES ($1) RETURNING id")
        .bind(did)
        .fetch_one(&mut *conn)
        .await?;
    Ok(row.get("id"))
...
pub async fn find_by_did(conn: &mut PgConnection, did: &str) -> Result<Option<Account>, sqlx::Error> {
    let row = sqlx::query("SELECT id, did FROM accounts WHERE did = $1")
        .bind(did)
