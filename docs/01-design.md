...
The substrate provides an **invitation token** primitive that gates and customizes account creation. An invitation token is a short-lived secret generated by a server admin or an existing user. It encodes: server URL, usage limit (single-use or multi-use), expiry, optional auto-enrollment into groups/Projects, an optional onboarding Project whose flow runs post-registration, and an optional inviter DID for auto-creating a DM. Tokens are encoded into deep links (`actnet://invite/...`) that can be shared as QR codes or sent via any platform.
